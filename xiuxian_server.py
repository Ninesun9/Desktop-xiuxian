...
def add_task():
    ...
    #print(opj_json)
    # 打开数据库连接
    db = MySQLdb.connect("127.0.0.1", "root", "123456", "xiuxian", charset='utf8mb4' ,port=3306)
    # 使用cursor()方法获取操作游标 
    cursor = db.cursor()
    
    # SQL 插入语句
    sql = "REPLACE INTO xiuxian (uuid,user_name,sheng_ming,gong_ji,fang_yu,wu_xing,xiu_wei,jing_jie,remote_addr,up_dt) VALUES( "
    sql += " '"+opj_json['userid']+"',"
...
def get_rank_xiuwei():
    ...
    try:
        ...
        for row in results:
            ...
            i=i+1
            table.append_data_rows((
                (str(i),str(row[0]),str(row[1]),str(row[2]),str(row[3]),str(row[4]),str(row[5])),
                ))
    except:
        # Rollback in case there is any error
        print(sql)
